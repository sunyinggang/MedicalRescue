package com.jointthinker.framework.web.bean;
...
public class ViewBean {
	...
	public void addViewBeanValue(String label, String value, int index){
		if (this.labellist!=null&&this.labellist.size()>0&&this.valuelist!=null&&this.valuelist.size()>0)
		{
			this.labellist.add(label);
			if(index == -1){
				for(int i=0;i<this.valuelist.size();i++){
					((List)(this.valuelist.get(i))).add(value);
				}
			}else{
				((List)(this.getValuelist().get(index))).add(value);
			}
		}
	}
