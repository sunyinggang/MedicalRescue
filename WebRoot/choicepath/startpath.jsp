...
<html>
  <head>
    ...
    <title>My JSP 'choicepath.jsp' starting page</title>
    	        	...
    	        	handler: function(e){
    	        		...
    	        		if (form.form.isValid()){
			      			 Ext.MessageBox.confirm('提示', '确定保存吗？', function(btn, text) {
			 		    		if(btn == 'yes') {
			     	        		if($('#Map_input_haddress').val()=="天津市"){
			     	        			Ext.MessageBox.alert("错误信息","地址不详细，请重新填写完整地址！");
			     	        			return;
			     	        		}console.log($('#Map_input_haddress').val());
			     	        		
			     	        		form.getForm().findField('haddress').setValue($('#Map_input_haddress').val()); 
			     	        		form.getForm().submit({
	            	...
	            	Ext.Ajax.request({
						...
						success:function(o){
							...
							$("#<%=nameSpaceId%>_case1").bMap({
			                    ...
			                    {
			                    	btn.up('panel').down('form').getForm().findField('lat').setValue(point.lat);
			                    	btn.up('panel').down('form').getForm().findField('lng').setValue(point.lng);
			                    }
			            	});
						}
	            	});
